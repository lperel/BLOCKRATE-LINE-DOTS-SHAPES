...
const state={phase:"idle",duration:null,blockDuration:null,current:null,previous:null,unresolvedStreak:0,overloads:[],recoveries:[],recoveryCorrectCompleted:0,history:JSON.parse(localStorage.getItem("blockrate_v21_version_fix_history")||"[]"),totalTrials:0,trialTimer:null,absoluteNoResponseTimer:null,lastFiveAnswers:[],samnPerelli:null,subjectId:null,calibrationTrialIndex:0,calibrationRTs:[],calibrationErrors:0,trialOpenedAt:null};
...
function loadSettings(){const s=JSON.parse(localStorage.getItem("blockrate_v21_version_fix_settings")||"null");return s?{...DEFAULTS,...s}:{...DEFAULTS}}
function saveSettings(){localStorage.setItem("blockrate_v21_version_fix_settings",JSON.stringify(settings))}
...
function goToStartPage(){
  clearCurrentSession();
  resultBox.textContent="V21 build active.";
...
End reason:
${result.endReason}`}
function exportResults(){const blob=new Blob([JSON.stringify({settings,history:state.history},null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="blockrate_v21_version_fix_results.json";a.click()}
function emailResults(){const last=state.history[state.history.length-1]||{},body=encodeURIComponent(JSON.stringify(last,null,2));window.location.href=`mailto:?subject=BlockRate v21&body=${body}`}
...
$("exportAdminBtn").onclick=()=>{const blob=new Blob([JSON.stringify(settings,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="blockrate_v21_version_fix_admin_export.json";a.click()};
