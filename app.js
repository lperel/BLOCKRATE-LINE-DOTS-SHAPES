...
function loadSettings(){const s=JSON.parse(localStorage.getItem("blockrate_v22_noresponse_fix_settings")||"null");return s?{...DEFAULTS,...s}:{...DEFAULTS}}
function saveSettings(){localStorage.setItem("blockrate_v22_noresponse_fix_settings",JSON.stringify(settings))}
...
function goToStartPage(){
  clearCurrentSession();
  resultBox.textContent="V22 build active.";
...
End reason:
${result.endReason}`}
function exportResults(){const blob=new Blob([JSON.stringify({settings,history:state.history},null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="blockrate_v22_noresponse_fix_results.json";a.click()}
function emailResults(){const last=state.history[state.history.length-1]||{},body=encodeURIComponent(JSON.stringify(last,null,2));window.location.href=`mailto:?subject=BlockRate v22&body=${body}`}
...
$("exportAdminBtn").onclick=()=>{const blob=new Blob([JSON.stringify(settings,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="blockrate_v22_noresponse_fix_admin_export.json";a.click()};
