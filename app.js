...
const DOT_PATTERNS = {
  1:[["dot",50,50]],2:[["dot",34,50],["dot",66,50]],3:[["dot",50,30],["dot",50,50],["dot",50,70]],
...
const LINE_PATTERNS = {
  1:[["v",50,50]],2:[["v",40,50],["v",60,50]],3:[["h",50,30],["h",50,50],["h",50,70]],
...
const state = {
  phase: "idle", // calibration | paced | recovery | terminal_recovery | finished
  duration: null,
  blockDuration: null,
  current: null,
  previous: null,
...
function validateTrial(trial){
  ...
  return correctIdx >= 0 && trial.upperItems[correctIdx].count === trial.targetCount;
}
...
function openTrial(kind){
  ...
  if(kind === "calibration"){
    ...
    setStatus(idx <= settings.initialUnusedCalibrationTrials ? "Unused self-paced trial — waiting for response" : "Measured self-paced trial — waiting for response");
  } else if(kind === "paced"){
...
function trialMatches(trial, index){
  return trial && trial.lowerShapes[index] === trial.correctShapeId;
}
...
Initial machine-paced duration = ${Math.round(settings.initialPacedPercent*100)}% of average = ${pacedStart.toFixed(1)} ms`;
...
function startTest(){
  ...
  resultBox.textContent = `Calibration starting.\n1 unused self-paced trial, then ${settings.initialMeasuredCalibrationTrials} measured self-paced trials.\nInitial machine-paced value will be calculated as ${Math.round(settings.initialPacedPercent*100)}% of the measured average and is no longer a manual admin default.`;
  noteAnyResponse();
  openTrial("calibration");
}
...
$("saveAdminBtn").addEventListener("click", ()=>{
  ...
  saveSettings();
  renderAdmin();
  setStatus("Admin settings saved as the new default on this device");
});
